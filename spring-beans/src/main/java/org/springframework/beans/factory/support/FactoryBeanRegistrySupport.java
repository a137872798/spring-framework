				...
				if (object == null) {
					...
					//这里是缓存容器中还没有数据时进入
						...
						if (shouldPostProcess) {
							...
							//执行钩子 就是将bean添加到标识正在创建的容器中
							beforeSingletonCreation(beanName);
							try {
								//对从工厂中获得的bean做后续处理  默认情况直接返回 子类重写了这个方法 也就是依次调用BeanPostProcessor的处理方法
